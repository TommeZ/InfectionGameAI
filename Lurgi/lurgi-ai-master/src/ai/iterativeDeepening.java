...
/**
 * Created with IntelliJ IDEA. User: DrMark Date: 11/18/13 Time: 11:12 PM To
...
public abstract class iterativeDeepening implements AI {

    /**
     * Inner class used to store a move, its eventual heuristic rating, and the
     * depth to which it was explored.
     */
    class RatedMove {
    ...
    /**
     * Cache entry - stores a game state, and the best move we found from that
     * state, to reuse in the future. The "staleness" value is the number of
     * empty squares on the board in this state - since in this game the number
     * of pieces on the board never goes down, we can use this to quickly clear
     * out useless cache entries.
     */
    class CacheEntry {
    ...
    // For statistics.
    int states, ratings, alphas, betas, hits, bumps, overdrives, crashes;
    ...
    int rateMove(State fromBoard, Move theMove, int level, int inAlpha, int inBeta) {
        ...
        while (currentTime <= endTime) {
            for (int i = 0; i <= level; i++) {
                RatedMove next = moveSearch(newBoard, level, inAlpha, inBeta);
                if (next != null) {
                    return next.rating;
                } else {
    ...
    RatedMove moveSearch(State fromBoard, int level, int inAlpha, int inBeta) {
        ...
        if (cache.containsKey(zobrist)) {
            ...
            boolean ok = true;
            if (test.whoseTurn() != fromBoard.whoseTurn()) {
                ok = false;
            }
            if (ok) {
                for (int x = 0; x < 10; x++) {
    ...
    public Move nextMove(State board) {
        ...
        System.out.println("Iterative Deepening examined " + states + " states, " + "made " + ratings + " ratings, " + alphas + " alpha cut-offs, " + betas + " beta cut-offs.");
        ...
        for (int key : cache.keySet()) {
            if (cache.get(key).staleness > threshold) {
                deadlist.add(key);
            }
        }
